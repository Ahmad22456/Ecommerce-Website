...
export async function productFiltersController(req, res) {
  // ???
  try {
...
// Product Number in Single Page Controller
export async function productListController(req, res) {
  try {
    const perPage = 1;
    const page = req.params.page ? req.params.page : 1;
    const products = await productModel // what are all these conditions ??
      .find({})
      .select("-photo")
      .skip((page - 1) * perPage)
      .limit(perPage)
